...
PROMPT_TARG="F"
MIPPATH="/tmp/install/questa/questa_sim-2020.4_ultra.aol"
...
MIPSKIP=
MIPEXTRAARGS=
MIPTARG="/opt/mentor/questa/2020.4"
MIPSRC="/tmp/install/questa/questa_sim-2020.4_ultra.aol"
while [ $# -gt 0 ];
do
   if [ "$1" = "-tgt" ]; then
      shift
...
if [ ! "$MIPSKIP" = "T" ]; then
   :
   "$MIPPATH" -silent -batchverify "$SELF" $NEWTARG $NEWMIPLOC -block
fi
if [ ! "$MIPSKIP" = "T" ]; then
   :
...
<batchFile>
	<source value="/tmp/install/questa/questa_sim-2020.4_ultra.aol"/>
	<target value="/opt/mentor/questa/2020.4"/>
